...
def growth_integ(cosmo, conf):
    ...
    def ode(G, lna, cosmo):
        a = jnp.exp(lna)
        dlnH_dlna = H_deriv(a, cosmo) - 1
        Omega_fac = 1.5 * Omega_m_a(a, cosmo)
        G1, G1p, G2, G2p = jnp.split(G, num_order * (num_deriv-1), axis=-1)
        r_xc = (cosmo.Omega_de * a**-(3 * (1 + cosmo.w_0) + cosmo.xi_)) / (cosmo.Omega_c * a**-3 + cosmo.Omega_de * a**-3 * ((cosmo.xi_/(3 * cosmo.w_0 + cosmo.xi_) * (1 - a**(-3 * cosmo.w_0 - cosmo.xi_)))))
        G1pp = -G1p * (dlnH_dlna + r_xc * cosmo.xi_ * a**-1 + 5) - G1 * (dlnH_dlna + r_xc * cosmo.xi_ * a**-1 + 4 - (Omega_fac - r_xc * cosmo.xi_ * dlnH_dlna + r_xc * cosmo.xi_ * a**-2 * (cosmo.xi_ + 3 * cosmo.w_0 + r_xc * cosmo.xi_ - a)))
        G2pp = -G2p * (4 + (dlnH_dlna + r_xc * cosmo.xi_ * a**-1 + 3)) - G2 * (4 + 2 * (dlnH_dlna + r_xc * cosmo.xi_ * a**-1 + 3) - (Omega_fac - r_xc * cosmo.xi_ * dlnH_dlna + r_xc * cosmo.xi_ * a**-2 * (cosmo.xi_ + 3 * cosmo.w_0 + r_xc * cosmo.xi_ - a))) + Omega_fac * G1**2
