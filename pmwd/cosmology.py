...
class Cosmology:
    ...
    Omega_c : float or jax.numpy.ndarray
    ...
    xi_ : None, float, or jax.numpy.ndarray, optional
        Dimensionless parameter governing the strength of the DM-DE interaction.
    ...
    w_0_fixed: ClassVar[float] = -1.01
    w_a_: Optional[FloatParam] = None
    w_a_fixed: ClassVar[float] = 0
    xi_: Optional[FloatParam] = None
    xi_fixed: ClassVar[float] = 0
    ...
    def xi(self):
        """Dark energy equation of state linear parameter."""
        return self.xi_fixed if self.xi_ is None else self.xi_
...
def E2(a, cosmo):
    ...
    de_a = a**-3 * (cosmo.xi_/(3 * cosmo.w_0 + cosmo.xi_) * (1 - a**(-3 * cosmo.w_0 - cosmo.xi_))) + a**-(3 * (1 + cosmo.w_0) + cosmo.xi_)
    return (cosmo.Omega_m - cosmo.Omega_b - cosmo.Omega_c) * a**-4 + cosmo.Omega_b * a**-3 + cosmo.Omega_c * a**-3 + cosmo.Omega_de * de_a
...
def Omega_m_a(a, cosmo):
    ...
    return (E2(a, cosmo))**-1 * (cosmo.Omega_m / (a**3) + (cosmo.Omega_de / (a**3)) * (cosmo.xi_/(3 * cosmo.w_0 + cosmo.xi_) * (1 - a**(-3 * cosmo.w_0 - cosmo.xi_))))
